...
sDF = pandas.DataFrame({'const' : 1/vix, 'vix' : 1})
nlargeReg = OLS(nlarge, sDF).fit()
...
nsmallDF = pandas.DataFrame({'const' : 1, 'large' : nlarge})
nsmallReg = OLS(nsmall - nlarge, nsmallDF).fit()
print(nsmallReg.summary())
resnsmall = nsmallReg.resid
print('stderr nominal small = ', numpy.std(sorted(resnsmall)[:-1]))
test(resnsmall, 'nominal small')
print('residual outlier = ', max(resnsmall))
rsmallDF = pandas.DataFrame({'const' : 1, 'large' : rlarge})
rsmallReg = OLS(rsmall - rlarge, rsmallDF).fit()
print(rsmallReg.summary())
resrsmall = rsmallReg.resid
print('stderr real small = ', numpy.std(sorted(resrsmall)[:-1]))
test(resrsmall, 'real small')
print('residual outlier = ', max(resrsmall))
